...
pub struct NotesApp {
    ...
    /// Ignore the next file system event generated by our own writes.
    ignore_next_fs_event: bool,
}
            ...
            if let (Some(f_idx), Some(n_idx)) = (self.selected, self.selected_note) {
                {
                    ...
                    if let Some(rx) = &self.rx {
                        while let Ok(evt) = rx.try_recv() {
                            if let Ok(ev) = evt {
                                if self.ignore_next_fs_event {
                                    self.ignore_next_fs_event = false;
                                    continue;
                                }
                        ...
                        .changed()
                    {
                        ...
                        self.ignore_next_fs_event = true;
